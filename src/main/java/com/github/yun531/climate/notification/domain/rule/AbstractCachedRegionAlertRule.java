...
public abstract class AbstractCachedRegionAlertRule<V> implements AlertRule {
     ...
     * 캐시 판단에 넣을 since.
     * - 기본: command.since()
     * - WarningIssuedRule 처럼 TTL을 "지금(now)" 기준으로 굴리고 싶으면 override 해서 now 반환  // todo WarningIssuedRule 확인
     */
    @Nullable
    ...
    protected abstract List<AlertEvent> buildEvents(
            String regionId,
            V value,
    ...
    /** NotificationService(상위 서비스)가 내려준 now 사용 */
    @Override
    public final List<AlertEvent> evaluate(GenerateAlertsCommand command, LocalDateTime now) {
        List<String> regionIds = (command == null) ? null : command.regionIds();
        ...
        LocalDateTime since = sinceForCache(command, effectiveNow);
        since = (since == null) ? null : TimeUtil.truncateToMinutes(since);

        List<AlertEvent> out = new ArrayList<>();

        for (String regionId : regionIds) {
            CacheEntry<V> entry = cache.getOrComputeSinceBased(
                    regionId,
                    since,
                    thresholdMinutes(),
            ...
            List<AlertEvent> events = buildEvents(
                    regionId,
                    entry.value(),
                    entry.computedAt(),
                    effectiveNow,
