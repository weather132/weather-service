...
public abstract class AbstractCachedRegionAlertRule<V> implements AlertRule {
    ...
    protected abstract CacheEntry<V> computeForRegion(String regionId);
    ...
    public List<AlertEvent> evaluate(NotificationRequest request) {
        ...
        LocalDateTime since = sinceForCache(request, now);
        ...
        for (String regionId : regionIds) {
            ...
            List<AlertEvent> events =
                    buildEvents(regionId, entry.value(), entry.computedAt(), now, request);
