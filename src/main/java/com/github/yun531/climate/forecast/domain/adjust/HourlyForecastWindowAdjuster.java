 ...
 * - reportTime 기준 최대 maxShiftHours(2) 까지 재사용
 * - validAt이 baseTime 초과(>)인 포인트만 최대 windowSize 개로 절단
 */
public final class HourlyForecastWindowAdjuster {
    ...
    /** 기준이 되는 ForecastHourlyView를 now 시각 기준으로 재조정 */
    public ForecastHourlyView adjust(ForecastHourlyView base, LocalDateTime now) {
        ...
        if (reportTime == null || now == null || sorted.isEmpty()) {
            return new ForecastHourlyView(base.regionId(), reportTime, sorted);
        }
