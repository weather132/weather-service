...
class WarningStateRepositoryTest {
    ...
    void findByRegionIdIn_여러지역조회() {
        WarningState a = repo.save(WarningState.builder()
                .regionId(101L).kind(WarningKind.RAIN).level(WarningLevel.ADVISORY).build());
        WarningState b = repo.save(WarningState.builder()
                .regionId(202L).kind(WarningKind.HEAT).level(WarningLevel.WARNING).build());
        WarningState c = repo.save(WarningState.builder()
                .regionId(303L).kind(WarningKind.WIND).level(WarningLevel.ADVISORY).build());

        // when
        List<WarningState> list = repo.findByRegionIdIn(List.of(101L, 202L));

        // then
        assertThat(list).extracting("regionId").containsExactlyInAnyOrder(101L, 202L);
    ...
    void findTopByRegionIdOrderByUpdatedAtDesc_최신한건_반환() {
        ...
        WarningState ws1 = repo.save(WarningState.builder()
                .regionId(777L).kind(WarningKind.RAIN).level(WarningLevel.ADVISORY).build());
        WarningState ws2 = repo.save(WarningState.builder()
                .regionId(777L).kind(WarningKind.RAIN).level(WarningLevel.WARNING).build());
        ...
        // when
        Optional<WarningState> top = repo.findTopByRegionIdOrderByUpdatedAtDesc(777L);
