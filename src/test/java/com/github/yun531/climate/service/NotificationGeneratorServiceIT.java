...
class NotificationGeneratorServiceIT {
    ...
    void only_rain_when_receiveWarnings_false() {
        var since = Instant.parse("2025-11-04T04:00:00Z");

        var messages = service.generate(List.of(1L), false, since);
    ...
    void rain_and_warnings_when_receiveWarnings_true() {
        ...
        var messages = service.generate(List.of(1L, 2L), true, since);
    ...
    void region_capped_to_three() {
        var since = Instant.parse("2025-11-04T04:00:00Z");
        var ids = List.of(10L, 11L, 12L, 13L); // 4개 입력

        // 실행
        service.generate(ids, EnumSet.of(AlertTypeEnum.RAIN_ONSET), since, Locale.KOREA);
