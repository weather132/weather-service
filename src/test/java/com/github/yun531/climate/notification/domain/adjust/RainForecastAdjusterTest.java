...
class RainForecastAdjusterTest {
    ...
    class NullGuards {
        ...
        @Test
        @DisplayName("null now -> 보정 없이 원본 반환")
        void nullNow_returnsOriginal() {
            RainInterval interval = new RainInterval(ANNOUNCE_TIME.plusHours(2), ANNOUNCE_TIME.plusHours(5));
            AlertEvent event = makeAlertEvent(List.of(interval));

            AlertEvent result = adjuster.adjust(event, ANNOUNCE_TIME, null);
    ...
    class WindowClipping {
        ...
        @Test
        @DisplayName("구간 end가 windowEnd를 넘으면 -> end를 windowEnd로 클리핑")
        void intervalExceedsWindowEnd_endClamped() {
            LocalDateTime now = ANNOUNCE_TIME; // 05:00
            // windowEnd = nowHour(05:00) + 24h = 다음날 05:00
            LocalDateTime windowEnd = now.plusHours(24);
            // 구간이 windowEnd를 넘어감
            RainInterval overlapping = new RainInterval(now.plusHours(22), now.plusHours(26)); // 03:00~07:00(다음날) -> end를 05:00으로 클리핑
            ...
            RainForecastPayload payload = (RainForecastPayload) result.payload();
            assertThat(payload.hourlyParts()).hasSize(1);
            assertThat(payload.hourlyParts().get(0).end()).isEqualTo(windowEnd);
        }
    }
    ...
    class MaxShift {
        ...
        @Test
        @DisplayName("now가 announceTime + 5h -> maxShift(2)로 클램프")
        void maxShiftClamp() {
            ...
            // shiftHours = min(5, 2) = 2, shiftedBaseTime = 05:00 + 2h = 07:00
    ...
    class DayShift {
        ...
        @Test
        @DisplayName("dayShift가 days 크기 이상이면 전부 EMPTY 패딩")
        void dayShiftExceedsDaysSize_allEmpty() {
            // maxShiftHours=2이므로 최대 dayShift=1이 가능
            // dayShift=1, days.size()=1 -> 전부 드롭 후 EMPTY 패딩
            RainForecastAdjuster customAdjuster = new RainForecastAdjuster(2, 24, 1);
            LocalDateTime announceTime = LocalDateTime.of(2026, 1, 22, 23, 0);
            LocalDateTime now = announceTime.plusHours(2);   // dayShift=1

            // days가 1개만 있음 -> dayShift=1이면 전부 드롭
            List<DailyRainFlags> singleDay = List.of(
                    new DailyRainFlags(true, true)
            );

            AlertEvent event = makeAlertEvent(List.of(), singleDay);
            AlertEvent result = customAdjuster.adjust(event, announceTime, now);

            RainForecastPayload payload = (RainForecastPayload) result.payload();
            // 원래 1개인데 dayShift=1 -> 전부 드롭, 1개 EMPTY 패딩
            assertThat(payload.dayParts()).hasSize(1);
